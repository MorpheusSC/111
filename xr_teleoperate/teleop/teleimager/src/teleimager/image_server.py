...
class IsaacSimCamera(BaseCamera):
    def __init__(self, cam_topic, img_shape, fps,
        ...
        super().__init__(cam_topic, img_shape, fps, enable_zmq, zmq_port, enable_webrtc, webrtc_port, webrtc_codec)
        try:
            from tools.shared_memory_utils import MultiImageReader  # Try importing from unitree_sim_isaaclab first
            logger_mp.info("[IsaacSimCamera] Using MultiImageReader from unitree_sim_isaaclab")
        except ImportError:
            from .multi_image_reader import MultiImageReader  # Fall back to local implementation
...
def run_isaacsim_server():
    ...
    try:
        ...
        exit(1)
    # Force all cameras to use isaacsim type for simulation
    for cam_key in cam_config:
        if cam_key.endswith('_camera'):
            cam_config[cam_key]['type'] = 'isaacsim'
            logger_mp.info(f"[run_isaacsim_server] Forcing camera {cam_key} to use isaacsim type")
    # Wait for IsaacSim shared memory to be ready
    logger_mp.info("[run_isaacsim_server] Waiting 2 seconds for shared memory to be ready...")
    time.sleep(2.0)
    # start image server
...
def main():
    ...
    parser.add_argument('--isaacsim', action = 'store_true', help = 'Enable IsaacSim simulation mode (reads from shared memory instead of physical cameras)')
    ...
    # If IsaacSim mode is enabled, force all cameras to use isaacsim type
    if args.isaacsim:
        for cam_key in cam_config:
            if cam_key.endswith('_camera'):
                cam_config[cam_key]['type'] = 'isaacsim'
                logger_mp.info(f"[main] Forcing camera {cam_key} to use isaacsim type for simulation mode")
        logger_mp.info("IsaacSim mode enabled. Waiting 2 seconds for shared memory to be ready...")
        time.sleep(2.0)  # Wait for IsaacSim shared memory to be ready

    # start image server
    server = ImageServer(cam_config, realsense_enable=args.rs, camera_finder_verbose=False, isaacsim_enable=args.isaacsim)
    server.start()
