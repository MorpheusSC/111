...
class TeleVuer:
    def __init__(self, use_hand_tracking: bool, binocular: bool=True, img_shape: tuple=None, display_fps: float=30.0,
        ...
        # Save configuration for subprocess recreation
        ...
        self.vuer.spawn(fn, start=False)
    ...
    def _vuer_run(self):
        # Re-create Vuer instance in subprocess because it cannot be pickled
        try:
            vuer = Vuer(host='0.0.0.0', cert=self._cert_file, key=self._key_file, queries=dict(grid=False), queue_len=3)
            vuer.add_handler("CAMERA_MOVE", self.on_cam_move)
            if self.use_hand_tracking:
                vuer.add_handler("HAND_MOVE", self.on_hand_move)
            else:
                vuer.add_handler("CONTROLLER_MOVE", self.on_controller_move)

            # Re-bind the spawn function
            if self.display_mode == "immersive":
                if self.webrtc:
                    fn = self.main_image_binocular_webrtc if self.binocular else self.main_image_monocular_webrtc
                elif self.zmq:
    ...
    def _xr_render_loop(self):
        while not self.stop_writer_event.is_set():
            ...
            if latest_frame is None:
                self.new_frame_event.clear()
                continue
            if hasattr(latest_frame, 'bgr'):
                latest_frame = latest_frame.bgr
            elif hasattr(latest_frame, 'rgb'):
                latest_frame = latest_frame.rgb
            latest_frame = np.asarray(latest_frame)
            if latest_frame.size == 0:
                self.new_frame_event.clear()
                continue
            if latest_frame.ndim == 3 and latest_frame.shape[-1] == 3:
    ...
    def _session_alive(self, session) -> bool:
        """Check whether current websocket session is still valid."""
        try:
            ws_id = getattr(session, "CURRENT_WS_ID", None)
            if ws_id is None:
                return False
            vuer_obj = getattr(session, "vuer", None)
            if vuer_obj is None:
                return False
            ws_pool = getattr(vuer_obj, "ws", None)
            if ws_pool is None:
                return False
            return ws_id in ws_pool
        except Exception:
            return False

    async def _safe_upsert(self, session, element, to="bgChildren") -> bool:
        """Safely upsert a scene element. Return False if session is gone."""
        try:
            ...
            print(f"[TeleVuer] upsert failed: {e}")
            return False
    ...
    async def main_image_monocular_zmq(self, session):
        ...
        while self._session_alive(session):
            ok = await self._safe_upsert(
                session,
                [
                    ImageBackground(
    ...
    async def main_image_monocular_webrtc(self, session):
        ...
        # WebRTC plane 只需要插入一次；后面只监测 session 存活
        while self._session_alive(session):
            await asyncio.sleep(1.0)
    ## ego MODE
